...
NTL.data.scatterplot <-
  ggplot(NTL.data.wrangled, aes(x=depth, y=temperature_C)) + geom_point() + geom_smooth(method=lm) +ylim(0,35) +xlab("Depth") +ylab("Temperature in Celsius")
...
> Answer: This figure shows that temperature decreases as the depth increases. 
...
NTL.AIC <- lm (data=NTL.data.wrangled, temperature_C ~ year4 + daynum + depth)
step(NTL.AIC)
...
>Answer: The t-test would allow us to compare means between the two lakes 
