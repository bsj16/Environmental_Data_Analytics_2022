...
editor_options: 
  chunk_output_type: console #output will go to the console
---
...
*Residual error should follow a normal distribution so it is symmetric. Probability of being higher or lower is the same* 
*R-squared: amount of variable, R-squared shows how well the x variable fits to the y variable. X variable shows X% of variability in y variable. How much of the variability in the series is explained by explanatory variable* 
*P-value: significance of the regression. Shows p-value in response to each coefficient. Want a small p-value (<0.05)*
*Increase in R-squared can be a result of adding another variable to the regression* 
*When R-squared is closer to zero, we add more variables* 

*Interaction variable-create a correlation between two variables* 
...
```{r}
irradiance.regression <- lm(PeterPaul.chem.nutrients$irradianceWater ~ PeterPaul.chem.nutrients$depth) #depth is the independent variable, irradiance is the dependent variable 

# another way to format the lm() function
irradiance.regression <- lm(data = PeterPaul.chem.nutrients, irradianceWater ~ depth)
summary(irradiance.regression) #-95.89 shows a negative relationship 

#Multiple R-squared --> what % variability is accounted for?
...
corrplot.mixed(PeterPaulCorr, upper = "ellipse") #displayed in upper diagonal
#Darker blues and darker reds show a stronger relationship 
#thinner line shows stronger relationship 
```
...
#Run linear regression on all variables 

*AIC helps us figure out what variables to have for model selection --> what regression is leading to the best fit?* 
...
#example- when removing po4, AIC is 352, AIC is 352.76 when removing depth. When AIC increases, we should keep the variable in the linear regression

#repeat AIC process until first variable is <none>, want to increase AIC of the model 
#Don't want variables with high correlation in model because we may be overfitting 
step(TPAIC)
TPmodel <- lm(data = Paul.naomit, tp_ug ~ dissolvedOxygen + temperature_C + tn_ug) #we look at the impacts of removing particular variables 
summary(TPmodel)
