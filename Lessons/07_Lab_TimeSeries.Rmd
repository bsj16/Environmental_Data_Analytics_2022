...
Rainfall_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% #space is new column in dataset, space is the separator 
...
Temp_BR_processed <-
  ...
  mutate( Date = my(paste0(Month,"-",Year))) %>% #paste0 means we are pasting data together 
...
#we want equally spaced observations in time series analysis   
...
Plot.Temp <- ggplot(BRA_complete, aes(y=Temperature_C, x=Date)) +geom_line() +geom_smooth(method="lm")
print(Plot.Temp)

Plot.Rainfall <-ggplot(BRA_complete, aes(y=Rainfall, x=Date)) +geom_line() + geom_smooth(method = "lm")
print(Plot.Rainfall) 
...
Rainfall.ts <- ts(Rainfall_BR_processed$Rainfall, start = c(1901,1), frequency = 12)
plot(Rainfall.ts)
#can also do fmonth and fyear for first month and year fmonth <-month(first(BRA_complete$Date))

Temp.ts <-ts(Temp_BR_processed$Temperature_C, start = c(1901,1), frequency = 12)
plot(Temp.ts)
...
Rainfall.components <- as.data.frame(Rainfall.decomposed$time.series[1,3])
Temp.components <- as.data.frame(Temp.decomposed$time.series[1,3])
