...
print(Plot_text) #play with this to see how expression changes
...
plot1 <- 
  ...
  facet_wrap(vars(lakename), nrow=3) + #facet wrap separtures the two lakes 
  ylab("Surface Temperature in Celsius") #changes label on y axis ="Year") 
+labs(color="Year")
+print(plot1)
...
plot2 <-
  ...
  ylab("Surface Temperature in Celsius")+
  scale_color_viridis(option ="magma") #change axis?
...
plot3 <- ggplot(EPAair, aes(x=PM2.5, y=Ozone)) + geom_point(aes(color=SITE_LATITUDE)) + geom_smooth(method=lm)
print(plot3)
