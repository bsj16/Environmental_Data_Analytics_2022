...
colnames(NTL.phys.data) #different entries for different columns 
head(NTL.phys.data)
...
# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] #filtering rows that have a depth of zero 
...
summary(NTL.phys.data$lakename) #filter only chooses rows 
...
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305) #can use , instead of &
...
NTL.phys.data1990to1999<-filter(NTL.phys.data, year4 %in% c(1990:1999))
...
```{r}
NTL.phys.data.temps <- select(NTL.phys.data, lakename, sampledate:temperature_C)  #last figure is for sample date to temperature_C
...
NTL.phys.data.PeterPaul1 <- mutate(NTL.phys.data.PeterPaul1, month = month(sampledate)) #we are returning the month for each observation

# reorder columns to put month with the rest of the date variables
NTL.phys.data.PeterPaul1 <- select(NTL.phys.data.PeterPaul1, lakeid:daynum, month, sampledate:comments) #moving month so it's by sample date

# find out the start and end dates of the dataset
interval(NTL.phys.data.PeterPaul1$sampledate[1], NTL.phys.data.PeterPaul1$sampledate[21613]) #when dataset is ordered by date 
interval(first(NTL.phys.data.PeterPaul1$sampledate), last(NTL.phys.data.PeterPaul1$sampledate)) #first date and last date, better approach 
```
...
```{r}
NTL.phys.data.processed <- #since it's a pipe, we don't need to provide a name for the dataset 
  NTL.phys.data %>%
