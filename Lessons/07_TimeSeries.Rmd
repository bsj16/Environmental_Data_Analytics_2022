...
Time series analysis can help for forecasting and address temporal trends 

Stationary Data- time series variable exhibiting no significant upward or downward trend over time
Nonstationary data- time series variable exhibiting signicant upward or downward trend over time
Seasonal data- time series variable exhibiting repeating patterns at regular intervals over time

Autocovariance and autocorrelation function (ACF) give info about the dependence structure of a time series 

Pt, t-h = Corr(Yt, Yt-h) measures linear dependency among the process variables Partial Autocorrelation Function (PACF)
Always build ACF and PACF before fitting a model to a stochastic process
ACFs help us identify seasonality in a dataset

We estimate the trend so we can remove it from the dataset

Generally assume linear trend to simplify analysis
...
```{r}
EnoDischarge.late_ts <- ts(EnoDischarge.late$discharge.mean, 
                           start = c(1985,1), frequency = 365) #increase by 1 day when viewing new observation
```
...
wind_data_clean <- 
  wind_data %>% 
  mutate( AWND.clean = zoo::na.approx(AWND) ) #uses an interpolation 

summary(wind_data_clean$AWND.clean)
#Note the NA is gone

#Do not use na.omit or drop.na for NAs in the middle of the dataset 
...
Difference between stochastic trend and deterministic trend-deterministic trend is constant over time, stochastic keeps the same direction but changes slowly over time. 
...
# Inspect results
wind_data_trend2
summary(wind_data_trend2) #stronger tendency for decrease for some seasons 
