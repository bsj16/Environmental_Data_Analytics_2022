...
O3plot5 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  mytheme #setting theme withing the plot environment
print(O3plot5)  
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  ...
  geom_text(x = as.Date("2020-01-01"), y = 120, label = "unhealthy (sensitive groups)", hjust = 1, fontface = "bold") + #we are defining different levels based on y value 
...
Yellow and red should be for warm and hot temperatures 
...
NvsP2 <-
  ...
  scale_shape_manual(values = c(15, 17)) + #numbers are equivalent to each shape 
...
library(cowplot)
plot_grid(NvsP2, Nutrientplot, nrow = 2, align = 'h', rel_heights = c(1.25, 1)) #two plots alongside each other 
...
```
Fill colors boxplot with same color, fill also colors contour of line 