...
> Answer: Ozone levels are greater than 50
...
ozone.density <- ggplot(EPAair, aes(x=Ozone, fill=as.factor(Year))) +
  geom_density(alpha=0.4) + #alpha is for transparency
  scale_fill_manual(values=c("green", "darkblue")) +
  labs(x="Ozone AQI value", y="Density", fill="") #fill-"" signals that we don't have a label for the legend 
print(ozone.density)

ozone.boxplot <- ggplot(EPAair, aes(x=as.factor(Year), y=Ozone)) +
  geom_boxplot() +
  stat_summary(geom = "text", fun.y = max, 
               vjust = -1, size = 4,
               label = c("b", "a")) + #vjust is vertical justification which places the label above the plot 
  labs(x="", y="Ozone AQI value")
print(ozone.boxplot)

ozone.violin <- ggplot(EPAair, aes(x=as.factor(Year), y=Ozone)) +
  geom_violin() + stat_summary(geom = "text", fun.y=max, 
                               vjust= -1, size =4,
                               label = c("b", "a")) +
  labs(x="", y="Ozone AQI Value")
print(ozone.violin)
...
EPA.corr <- cor(EPA.filtered)
corrplot(EPA.corr, method="ellipse")
corrplot.mixed(EPA.corr, upper ="ellipse")
...
PM2.5.regression <- lm(data=EPA.filtered, PM2.5 ~ meanLat + meanLng + Month + Year + Ozone)

                         #Increasing AIC means more variables are being added 
