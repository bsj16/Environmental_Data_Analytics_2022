...
vector2
vector3 <- c(TRUE,TRUE,TRUE,FALSE,TRUE,FALSE) #logical vector, must be capital 
vector3

vector1[3] #return 1 value in vector
...
matrix1[4, ] #row 4, blank means all columns 4th row of matrix 1
matrix1[ , 3] #returns values in column 3
matrix1[c(12, 14)] #returns 12 and 14
matrix1[c(12:14)] #returns 12-13
matrix1[2:4, 1:3] #rows 2-4, columns 1-3
...
dataframe1 <- data.frame(d,e,f) #joins all the vectors into a dataframe
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # 

dataframe1[1:2,] #rows 1-2 and all columns
dataframe1[c("ID","Passed")] #pulls columns with certain title 
dataframe1$ID

```
Question: How do the different types of data appear in the Environment tab?

> Answer: Vectors have a string of numbers, data frames have observations of variables 

Question: In the R chunk below, write "dataframe1$". Press `tab` after you type the dollar sign. What happens?


> Answer: You see options for each column 
...
```{r}
High.Durham.F <- c(49,58,39,41,44,37,41,51,52,52)
Low.Durham.F <- c(30,30,18,26,26,18,26,26,28,39)
...
```{r}
Celsius.Conv<-function(x){x-32 * 5/9}


 Celsius.Conv(High.Durham.F)
 Celsius.Conv(Low.Durham.F)
 
 
Durham.High.C <-Celsius.Conv(High.Durham.F)
Durham.Low.C <-Celsius.Conv(Low.Durham.F)
...
```{r}
Durham.Temps <- data.frame("HighF"=High.Durham.F, "LowF"=Low.Durham.F, "HighC"=Durham.High.C, "LowC"=Durham.Low.C)
Durham.Temps
...
```{r}
summary(Durham.Temps)
sd(Durham.Temps$HighF)
...
today <- Sys.Date()
format(today, format = "%B") #unabbreviated month
format(today, format = "%y") #abbreviated year
format(today, format = "%m-%d-%Y") #01 30 2022
#format(today, format = "")
