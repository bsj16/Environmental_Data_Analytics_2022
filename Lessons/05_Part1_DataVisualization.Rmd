...
Geoms add an additional layer to the plot 
...
ggplot(EPAair, aes(x = Date, y = Ozone)) + 
  geom_point() #generates points 

O3plot <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone))
print(O3plot) #store plot in an object 

# Fix this code
O3plot2 <- ggplot(EPAair) +
  #geom_point(aes(x = Date, y = Ozone, color = "blue")) color is not blue
...
PMplot <- 
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year), color = 
  Site.Name)) +
  geom_point()
print(PMplot) #color coded by site name, different shapes for each year 
...
PMplot.line <- 
  ggplot(subset(EPAair, Site.Name == "Leggett"), 
         aes(x = Date, y = PM2.5)) +
  geom_line() #all observations connected by line
print(PMplot.line)
...
print(Nutrientplot3) #why does plot have <<<Head? 
