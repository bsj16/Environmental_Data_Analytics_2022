...
```{r importdata}
<<<<<<< HEAD
# Absolute file path (not recommended), won't work because this file is not on my computer 
...
str(USGS.flow.data)
dim(USGS.flow.data) #number of rows and columns (dimensions)
length(USGS.flow.data) #how many columns are present

head(USGS.flow.data) #first few rows
head(USGS.flow.data, 10) #first ten rows of dataset
tail(USGS.flow.data, 5) #last five rows of dataset
USGS.flow.data[30000:30005, c(3, 8, 14)] #rows 30000-30005, columns 3,8,14

class(USGS.flow.data$datetime) #tells us what type of data each variable is 
...
create.early.dates <- (function(d) {
       paste0(ifelse(d > 191226,"19","20"),d) #191226-Dec 26, 2019, if before -> 2019, if else ->2020
       })
#
USGS.flow.data$datetime <- create.early.dates(USGS.flow.data$datetime)

#
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%Y%m%d") #convert back to date
