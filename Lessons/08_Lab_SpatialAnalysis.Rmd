...
#Create a map
mapview(huc8_sf, col.regions = 'orange') + mapview(upperNeuse_sf, col.regions ='purple')
...
Wind.data <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv',
                      colClasses = c('HUC_12'= 'factor')) %>%
  mutate(HUC_8=substr(HUC_12,1,8)) %>%
  group_by(HUC_8) %>%
  summarise(TotalWindEnergy =sum(kWhkm2day))
...
huc8_sf_wind <- huc8_sf %>% 
  left_join(Wind.data, by= c('HUC_8', 'HUC_8')) 
...
selected_HUCS <- huc8_sf_utm[triangle_sf,] #triangle_sf is the cookie cutter 
