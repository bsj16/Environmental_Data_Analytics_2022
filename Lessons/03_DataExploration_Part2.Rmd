...
#lets you zoom into histogram and see certain values 
...
#here we are coloring values based off of mean approval height 
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#boxplot horizontal line is the mean of values 
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean.approval, y = gage.height.mean))

#group command is present here, allows us to see different slices of the data, wider distribution at higher gauge heights  
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

#examines range of values and distribution within the range of values, data points distributed along the median  
ggplot(USGS.flow.data) +
